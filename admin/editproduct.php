...
if (isset($_GET['newcode']) && $id==0) {
    $product->item_code($_GET['newcode']);
}
...
if (isset($_GET['save']) && $_GET['save'] == 1) {
    ...
    if ($err == 0) {
        ...
        $product->item_code(trim($_POST['t_item_code']));
...
//        $product->compatible_machines_ids($_POST['t_compatible_machines_ids']);
        ...
        //check relative products / non existing - inactive
        $arNewCodes = array();
        $arNewCodesExt = array();
        ...
        $accessoriesCodes = km_functions::getArrayFromStrIds($product->accessories_ids());
        km_functions::checkCodes($accessoriesCodes, $arNewCodes, $arNewCodesExt, $arExistingCodes, 
                $arInactiveCodesExt, $db1);
        
        $relatedCodes = km_functions::getArrayFromStrIds($product->related_product_ids());
        km_functions::checkCodes($relatedCodes, $arNewCodes, $arNewCodesExt, $arExistingCodes, 
                $arInactiveCodesExt, $db1);
        
        $cleaningagentsCodes = km_functions::getArrayFromStrIds($product->cleaning_agents_ids());
        km_functions::checkCodes($cleaningagentsCodes, $arNewCodes, $arNewCodesExt, $arExistingCodes, 
                $arInactiveCodesExt, $db1);
        
        $standardAccessoriesCodes = km_functions::getArrayFromStrIds($product->standard_accessories_ids());
        km_functions::checkCodes($standardAccessoriesCodes, $arNewCodes, $arNewCodesExt, $arExistingCodes, 
                $arInactiveCodesExt, $db1);
        
        if ($arNewCodes || $arInactiveCodes) {
            $msg .= "<br/><br/>Υπάρχουν νέοι ή ανενεργοί σχετιζόμενοι κωδικοί. <br/><br/>";
            $msg .= "<a class=\"button\" href=\"findNewItems.php?itemid=$id\">Διαχείριση σχετιζόμενων κωδικών</a>";
        }
    ...
    /*========================================================================*/
    $accessoriesCatId = 18; /*======*/
    $sql = "SELECT * FROM app_categories WHERE parent_id = $accessoriesCatId "
            . "OR id=$accessoriesCatId";
    $myRS = $db1->getRS($sql);
    $catIdsAcc = MyUtils::get_ids($myRS, "id", ",");
    //....
    $cleaningAgentsCatId = 41; /*======*/
    $sql = "SELECT * FROM app_categories WHERE parent_id = $cleaningAgentsCatId "
            . "OR id=$cleaningAgentsCatId";
    $myRS = $db1->getRS($sql);
    $catIdsClean = MyUtils::get_ids($myRS, "id", ",");
    //....
    $procuctCatId = str_replace(array("[","]"), "", $product->category_id());
    
    $old_ids = $compatible_machines_ids;
    $new_ids = $_POST['t_compatible_machines_ids'];
    if ($old_ids!=$new_ids) {
        //new
        $sql = "SELECT * FROM app_products WHERE item_code IN ($new_ids)";
        $myRs = $db1->getRS($sql);
        for ($i = 0; $i < count($myRs); $i++) {
            $myItemCode = $myRs[$i]['item_code'];
            if (!func::strInStr($myItemCode, $old_ids)) {
                $device = new app_products($db1, $myRs[$i]['id'], $myRs);                             if (func::strInStr($procuctCatId, $catIdsAcc)) {
                    $str = $device->accessories_ids();
                    $str = func::ConcatSpecial($str, "[" . 
                            $product->item_code()."]", ",");
                    $str = str_replace(",,", ",", $str);
                    $device->accessories_ids($str);
                }
                //echo "catIdsClean=".$catIdsClean."<br/>";
                //echo "procuctCatId=".$procuctCatId."<br/>";
                if (func::strInStr($procuctCatId, $catIdsClean)) {
                    $str = $device->cleaning_agents_ids();
                    $str = func::ConcatSpecial($str, "[" . 
                            $product->item_code()."]", ",");
                    $str = str_replace(",,", ",", $str);
                    $device->cleaning_agents_ids($str);
                    //echo "cleaning_agents_ids";
                }                
                $device->Savedata();
            }        
        }       
        //deleted
        $sql = "SELECT * FROM app_products WHERE item_code IN ($old_ids)";
        $myRs = $db1->getRS($sql);
        for ($i = 0; $i < count($myRs); $i++) {
            $myItemCode = $myRs[$i]['item_code'];
            //echo $new_ids."<br/>";
            if (!func::strInStr($myItemCode, $new_ids)) {
                //echo $myItemCode."<br/>";
                $device = new app_products($db1, $myRs[$i]['id'], $myRs);
                if (func::strInStr($procuctCatId, $catIdsAcc)) {
                    $str = $device->accessories_ids();
                    $str = func::strRemoveItemFromIds($product->item_code(), $str);
                    /*
                    $str = str_replace("[".$product->item_code()."]", "", $str);
                    $str = str_replace(",,", ",", $str);
                    if (substr($str, 0,1)==",") {
    ...
    $compatible_machines_ids = $new_ids;
    /*========================================================================*/
...
//                        $c_accessories_ids->get_comboBox();
			...
			$t_standard_accessories_ids = new textbox("t_standard_accessories_ids", "Standard accessories", $standard_accessories_ids, "");
                        ...
                        $t_compatible_machines_ids = new textbox("t_compatible_machines_ids", "Compatible machines", $compatible_machines_ids, "");
                        $t_compatible_machines_ids->set_multiline();
                        $t_compatible_machines_ids->get_Textbox();
