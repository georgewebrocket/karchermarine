...
class func
{
    ...
    static function vlookupRS($fieldname, $rs, $idval, $idfield = "id") {        
        for ($i = 0; $i < count($rs); $i++) {
            if ($rs[$i]["$idfield"] == $idval) {
    ...
    static function strInStr($needles, $haystack) {
        $ar = explode(",",$needles);
        for ($i = 0; $i < count($ar); $i++) {
            if ($ar[$i]!="") {
                if (strpos($haystack, $ar[$i])!==FALSE) {
                    return TRUE;
                } 
            }
        }
    ...
    static function strRemoveItemFromIds($id, $ids) {
...
{	
    ...
    static function get_ids($array, $index, $unionstr) {
        ...
        for ($i=0;$i<count($array);$i++) {
            $my_ids = self::myconcat($my_ids, $array[$i][$index], $unionstr);
        }
        return $my_ids;
    }
...
class km_functions
{
    ...
    static function getArrayFromStrIds($strIds) {
    ...
    static function checkCodes($arrCodes, &$arNewCodes, &$arNewCodesExt, &$arExistingCodes, 
