...
<head>
    <meta charset="UTF-8">
    <title><?php echo $page->get_custom_field('SEO-PAGE-TITLE'); ?></title>
    <meta name="Description" content="<?php echo $page->get_custom_field('SEO-PAGE-DESCRIPTION'); ?>" />
	<meta name="Keywords" content="<?php echo $page->get_custom_field('SEO-PAGE-KEYWORDS'); ?>" />
    
	
    <meta property="og:title" content="<?php echo $page->get_custom_field('SEO-PAGE-TITLE'); ?>" />
    <meta property="og:image" content="<?php echo $page->get_feature_image(); ?>" />
	<meta property="og:image:url" content="<?php echo $page->get_feature_image(); ?>" />
	<meta property="og:description" content="<?php echo $page->get_custom_field('SEO-PAGE-DESCRIPTION'); ?>" />
	...
	function setSlideshow() {
		$('#home-ss').html($('#home-ss-content').html());
        ...
        <div class="wraper">
            <div class="col-12">
                <div class="slideshow" style="padding: 0px;">        
                   <div id="home-ss"></div> 
                        <div id="home-ss-content" style="display:none">
                        <ul class="bjqs">
                            ...
                            echo '<div class="banner">';
                                echo '<a href="'.$banner_sec_link.'">';
                                    echo '<div class="banner-bg" >';
                                        if (count($banner_sec_parts)>=9) {
											$mobileBg = trim($banner_sec_parts[8]);
											echo '<img class="visible-12 visible-sm-0" src="'.$banner_sec_img.'"/>';
											echo '<img class="visible-0 visible-sm-12" src="'.$mobileBg.'"/>';
										}
										else {
											echo '<img src="'.$banner_sec_img.'"/>';
										}
										
										echo '<div class="banner-text" style="color:'.$banner_sec_text_color.';' .$banner_sec_text_bottom.'">';
										echo '<h2>'.$banner_sec_text1.'</h2>';
                                        echo '<p>'.$banner_sec_text2.'</p>';
                                        echo '</div>';
                                        
										
										if (count($banner_sec_parts)>=10) {
											$productImg = trim($banner_sec_parts[9]);
											echo '<div class="visible-12 visible-sm-0" style="position:absolute; right:10px; bottom:-40px; width:15%; height:auto">';
											echo '<img src="'.$productImg.'"/>';
                                        	echo '</div>';
											
											echo '<div class="visible-0 visible-sm-12" style="position:absolute; right:10px; bottom:-30px; width:25%; height:auto">';
											echo '<img src="'.$productImg.'"/>';
                                        	echo '</div>';
										...
										$cardOverlayHeight = "";		
                                        if (count($banner_sec_parts)>=10) {
											$cardOverlayHeight = "style=\"height:115%\"";
										}
                                        if($banner_sec_text_overlay != ''){
                                        echo '<div class="card-overlay" '.$cardOverlayHeight.'>';
                ...
                <div class="news" >
                    ...
                    <div id="home-news" class="col-12 col-sm-0">
                        <ul class="bjqs">
							...
							foreach ($allnews as $post) { 
								if ($myCount % 3 == 0) { echo '<li>'; }
								
								echo '<div class="col-4">';
								echo '<div class="slide-post" style="padding-right: 30px;">';
								$date=date_create(str_replace("/","-",$post->get_post_date()));
								echo '<p class="slide-news-date" style="color: #757575;">'.date_format($date,"d. F Y").'</p>';
								echo '<h3 style="font-size: 16px; color:#2B2B2B; margin-bottom: 10px;"><a href="'.$post->get_post_name().'">'.$post->get_custom_field('H1').'</a></h3>';
								echo '<p class="post-content" style="font-size: 13px; color:#2B2B2B;">'.$post->get_post_excerpt().'</p>';
								echo '<a href="'.$post->get_post_name().'">More</a>';
								echo '</div>';
                                echo '</div>';
								
								if ($myCount % 3 == 2 || $myCount==count($allnews)-1) { echo '</li>'; }
								$myCount++;
							}
                    ...
                    <div class="col-0 col-sm-12">
						...
						foreach ($allnews as $allnew) { 
							if ($i<3) {
								echo '<h3 style="font-size: 16px; color:#2B2B2B; margin-bottom: 10px;"><a href="'.$allnew->get_post_name().'">'.$allnew->get_custom_field('H1').'</a></h3>';
            ...
            <div class="col-12">    
                <div class="partners">
                    <!--<div class="col-12">-->
                        <a href="customers"><h2 style="font-size:16px; font-weight:bold; margin-bottom:20px;">OUR CUSTOMERS</h2></a>
