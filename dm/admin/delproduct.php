...
if (isset($_GET['del']) && $_GET['del'] == 1) {
    ...
    if($product->Delete()) {        
        $rs = $db1->execSQL("DELETE FROM app_product_attributes WHERE product_id=?", 
		array($id));
        $id = 0;
        $msg = "<h2 style='color:green;' class='msg'>Προϊόν διαγράφτηκε με επιτυχία</h2>";
    }
    else{
...
<html>
	...
	<body>
            <div class="wrap">
                <div class="content">
                    <div class="col-12">
                        <h1>Διαγραφή προϊόντος</h1>
                        <?php if ($msg!="") { echo $msg;}  ?>
                        <form id="formDel" action="delproduct.php?id=<?php echo $id; ?>&del=1" method="POST">
                            <?php
                            //submit
                            if(!isset($_GET['del'])){
