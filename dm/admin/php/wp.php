...
public function get_post_content($wpstyle = false) {
    if ($wpstyle) {
        $str = $this->_post_content;
              $str = str_replace("\r\n\r\n","</p><p>", $str);
        $str = str_replace("\r\n","<br/>", $str);
...
class wp_category
{
	...
	//post_name -> slug
    public function __construct($myconn, $_id, $tprefix = "wp_", $order_by = "post_name", $limit = 0, $offset = 0, $my_rows = NULL) {
        $all_rows = NULL;
		$this->_myconn = $myconn;
		$this->_tprefix = $tprefix;
		...
		foreach ($all_rows as $posts){
			$arr_posts[] = new wp_post($this->_myconn, $posts['ID'], $this->_tprefix, $all_rows, "");
		}
...
protected $_dbo, $_post_id, $_rs;

public function __construct($dbo, $_postid, $tbprefix = "wp_") { 
     
    $this->_post_id = $_postid; 
    $this->_dbo = $dbo;
	$sql = "SELECT * FROM ".$tbprefix."postmeta WHERE post_id=?"; 
    $this->_rs = $this->_dbo->getRS($sql, array($_postid));   
	
}

public function getMetaValue($metaKey) {
	for ($i=0;$i<count($this->_rs);$i++) {
		if ($this->_rs[$i]['meta_key']==$metaKey) {
			return $this->_rs[$i]['meta_value'];
		}
	}
}
...
public function __construct($myconn, $_id, $tprefix = "wp_", $my_rows = NULL, $_ssql = '') { 
    ...
    if ($my_rows==NULL) { 
        $ssql = "SELECT * FROM ".$this->_tprefix."terms WHERE term_id=?"; 
        $all_rows = $this->_myconn->getRS($ssql, array($_id)); 
        } 
    else if ($_ssql!='') { 
        $ssql = $_ssql; 
    ...
    if ($icount==1) { 
        ...
        $this->_slug = $all_rows[0]['slug']; 
        $this->_term_group = $all_rows[0]['term_group']; 
    } 
} 
...
public function get_term_group() { 
    return $this->_term_group; 
} 
public function set_term_group($val) { 
    $this->_term_group = $val; 
} 

public function Savedata() { 
    if ($this->_id==0) { 
    $ssql = "INSERT INTO ".$this->_tprefix."terms ( 
    name,
    slug,
    term_group
    ) VALUES (?, ?, ?)"; 
    $result = $this->_myconn->execSQL($ssql, array( 
        ...
        $this->_slug, 
        $this->_term_group)); 
    $ssql = $this->_myconn->getLastIDsql($this->_tprefix."terms");

        $newrows = $this->_myconn->getRS($ssql); 
        $this->_id = $newrows[0]['id']; 
    } 
    else { 
        $ssql = "UPDATE ".$this->_tprefix."terms set 
        name = ?, 
        slug = ?, 
        term_group = ?
        WHERE term_id = ?"; 
...
public function Delete() { 
    $ssql = "DELETE FROM ".$this->_tprefix."terms WHERE term_id=?"; 
    $result = $this->_myconn->execSQL($ssql, array($this->_id)); 
    if ($result===false) { 
        return false; 
    } 
...
class wp_menu
{
	...
	public function __construct($dbo, $menuid, $tprefix = "wp_") {
		
		$tb = $tprefix;
		$term_taxonomy_id = func::vlookup("term_taxonomy_id", $tb.'term_taxonomy', "term_id=$menuid", $dbo);
		$sqlPostsIDS = "SELECT object_id FROM ".$tb."term_relationships WHERE term_taxonomy_id = $term_taxonomy_id";
		$sqlPost = "SELECT ID, menu_order, post_title, '' AS parent_id, '' AS the_link, '' AS the_target, '' AS item_type, '' AS object_id FROM ".$tb."posts WHERE ID IN ($sqlPostsIDS) ORDER BY menu_order";
		$rsPost = $dbo->getRS($sqlPost);
		
		for ($i=0;$i<count($rsPost);$i++) {
			$postid = $rsPost[$i]['ID'];
			$postmeta = new wp_postmeta($dbo, $postid, $tb);
			$rsPost[$i]['parent_id'] = $postmeta->getMetaValue('_menu_item_menu_item_parent');
			$rsPost[$i]['the_link'] = $postmeta->getMetaValue('_menu_item_url');
			$rsPost[$i]['the_target'] = $postmeta->getMetaValue('_menu_item_target');
			$rsPost[$i]['item_type'] = $postmeta->getMetaValue('_menu_item_object');
			$rsPost[$i]['object_id'] = $postmeta->getMetaValue('_menu_item_object_id');
			switch($rsPost[$i]['item_type']) {
				case 'page':
				case 'post':
					$post = new wp_post($dbo, $rsPost[$i]['object_id'], $tb);
					//link
					$seo_friendly_url = $post->get_custom_field('SEO-FRIENDLY-URL');
					if ($seo_friendly_url!='') { $rsPost[$i]['the_link'] = $seo_friendly_url; }
					else { $rsPost[$i]['the_link'] = $post->get_post_name(); }
					$h1 = $post->get_custom_field('H1');
					//TITLE
					if ($rsPost[$i]['post_title']=='') {
						if ($h1!='') { $rsPost[$i]['post_title'] = $h1; }
						else { $rsPost[$i]['post_title'] = $post->get_post_title(); }
					}			
					
					break;		
				case 'category':
					$cat = new wp_cat($dbo, $rsPost[$i]['object_id'],$tb, NULL, ''); 
					$rsPost[$i]['the_link'] = $cat->get_slug();
					if ($rsPost[$i]['post_title']=='') {
						$rsPost[$i]['post_title'] = $cat->get_name();
					}
					break;
				case 'custom':
					break;
		...
		$rsPost0 = array_filter($rsPost, function($el) { return $el['parent_id']==0; });
	...
	public function GetMenu($levels = FALSE, $attr = '', $ids = FALSE) {
		self::doGetMenu($this->rs0, $this->rs, $levels, 0, $attr, $ids);
	}
	
	static function doGetMenu($rs0, $rs, $levels = FALSE, $level=0, $attr = '', $ids) {
		$numItems = count($rs0);
		
		if ($levels) {  $level++; }
		
		if ($numItems>0) { echo "<ul ".$attr." >"; }
		
		foreach ($rs0 as $item) {
			
			$theLink = $item['the_link'];
			$theTitle = $item['post_title'];
			$class = ''; $elid = '';
			if ($levels) { $class = "class=\"level$level\""; }
			if ($ids) { $elid = "id=\"$theLink\""; }
			if ($theLink=='#') {
				$actual_link = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
				echo "<li $class $elid ><a href=\"$actual_link\">$theTitle</a>";
			}
			else {
				echo "<li $class $elid ><a href=\"$theLink\">$theTitle</a>";
			}
			
			$myId = $item['ID'];
			$rsX = array_filter($rs, function($el) use($myId) { return $el['parent_id']==$myId; });
			
			self::doGetMenu($rsX, $rs, $levels, $level, '', $ids);
			
			echo "</li>";
		
		}
		
		if ($numItems>0) { echo '</ul>'; }
	}
