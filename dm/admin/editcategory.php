...
if (isset($_GET['save']) && $_GET['save'] == 1) {
    ...
    /*if ($_FILES['UploadPhoto']['tmp_name']!="") {
        ...
        if ($_FILES['UploadPhoto']['size'] > 2097152){
            $msg .= "Έχεις ξεπεράσει το μέγεθος των 2 MB. Δοκίμασε μια άλλη φωτογραφία.<br>";
            $err = 1;
        }

        $types = array('image/jpeg', 'image/gif', 'image/png');

        if (in_array($_FILES['UploadPhoto']['type'], $types)) {

        } else {
            $msg .= "H φωτογραφία σου θα πρέπει να είναι της μορφής .JPG, .JPEG ή .PNG.";
            $err = 1;
        } 
    }*/
    ...
    if ($err == 0) {
        ...
        /*if ($_FILES['UploadPhoto']['tmp_name']!="") {
            $image_filesize = filesize($_FILES['UploadPhoto']['tmp_name']); 
            $image = new SimpleImage();
            $image->load($_FILES['UploadPhoto']['tmp_name']); 
            $photo = $_FILES['UploadPhoto']['name'];
            $mypath = "uploads/";
            $image->save($mypath . $photo);
            $category->photo(app::$host.$mypath . $photo);
        } */
            ...
            <div class="content">
                <div class="col-12">
                    ...
                    <form id="editForm" class="editForm" action="editcategory.php?id=<?php echo $id; ?>&ParentId=<?php echo $ParentId; ?>&save=1" method="POST" enctype="multipart/form-data">
                        ...
                        //short description
                        $t_short_description = new textbox("t_short_description", "ΠΕΡΙΓΡΑΦΗ", $short_description, "");
