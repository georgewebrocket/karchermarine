	...
	<script>
		var FileBrowserDialogue = {
		  ...
		  var SetFeatureImage = {
			...
			mySubmit: function (URL) {
			  // pass selected file path to TinyMCE
			  $('#t_photo', window.parent.document).val(URL);
                          $('#img-photo', window.parent.document).attr('src', URL);
			...
			mySubmit: function (URL) {
			  // pass selected file path to TinyMCE
			  $('#t_product_image', window.parent.document).val(URL);
			  $('#img-product-image', window.parent.document).attr('src', URL);
			...
			mySubmit: function (URL) {
			  // pass selected file path to TinyMCE
                          var str_extra_images = $('#t_extra_images', window.parent.document).val();
			  $('#t_extra_images', window.parent.document).val(str_extra_images + URL);
//			  $('#img-extra_images', window.parent.document).attr('src', URL);
			...
			mySubmit: function (URL) {
			  // pass selected file path to TinyMCE
                          var str_extra_images = $('#t_downloads', window.parent.document).val();
			  $('#t_downloads', window.parent.document).val(str_extra_images + URL);
//			  $('#img-extra_images', window.parent.document).attr('src', URL);
	  ...
	  $().ready(function() {
		var elf = $('#elfinder').elfinder({
		  ...
		  getFileCallback: function(file) { // editor callback
			...
			var str = "http://karcher-marine.com/wp/wp-content/" + file.path;
			<?php
			if ($mode=="tinymce") {
			?>
			FileBrowserDialogue.mySubmit(str); // pass selected file path to TinyMCE 
			<?php
			} elseif ($mode=="featuredimage") {
			?>
			SetFeatureImage.mySubmit(str);
			<?php
			} elseif ($mode=="productimage") {
			?>
			SetProductImage.mySubmit(str);
			<?php
			} elseif ($mode=="extraimages") {
			?>
			SetExtraImages.mySubmit(str);
			<?php
			} elseif ($mode=="pdf_downloads") {
			?>
			SetPDFDownloads.mySubmit(str);
			<?php
			} 
			?>
		  }
